...
def train(data_name: str, classes_per_node: int, num_nodes: int,
          steps: int, inner_steps: int, optim: str, lr: float, inner_lr: float,
          embed_lr: float, wd: float, inner_wd: float, embed_dim: int, hyper_hid: int,
          n_hidden: int, n_kernels: int, bs: int, device, eval_every: int, 
          seed: int) -> None:
    
    nodes = Basenodes(data_name, num_nodes, classes_per_node = classes_per_node, batch_size = bs)
    embed_dim = embed_dim
    if embed_dim == -1:
    ...
    for step in step_iter:
        ...
        for i in range(inner_steps):
            ...
            pred = net(img)
            label = label.squeeze()
            # print(f"The size of the labels is {label.shape}")
            loss = criteria(pred, label)
            loss.backward()

            torch.nn.utils.clip_grad_norm_(net.parameters(), 5000)
        ...
        torch.nn.utils.clip_grad_norm_(hnet.parameters(), 500)
        optimizer.step()

        step_iter.set_description(
            f"Step: {step+1}, Node ID: {node_id}, Loss: {prvs_loss},  Acc: {prvs_acc}"
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--data-name", type=str, default="pathmnist", choices=['pathmnist'], help="dir path for MNIST dataset"
    )
    ...
    parser.add_argument("--inner-lr", type=float, default=1e-5, help="learning rate for inner optimizer")
    parser.add_argument("--lr", type=float, default=1e-4, help="learning rate")
    parser.add_argument("--wd", type=float, default=1e-5, help="weight decay")
    parser.add_argument("--inner-wd", type=float, default=1e-7, help="inner weight decay")
    parser.add_argument("--embed-dim", type=int, default=-1, help="embedding dim")
    parser.add_argument("--embed-lr", type=float, default=None, help="embedding learning rate")
    parser.add_argument("--hyper-hid", type=int, default=100, help="hypernet hidden dim")
    parser.add_argument("--spec-norm", type=str2bool, default=False, help="hypernet hidden dim")
    parser.add_argument("--nkernels", type=int, default=64, help="number of kernels for cnn model")
